...
import * as hooks from '../../src/geographic-address-validation/geographic-address-validation.hooks';
...
const AT_TYPE = 'GeographicAddressValidation';
/** One the payload supplies instead, which must survive that default. */
const SUPPLIED_AT_TYPE = 'CustomGeographicAddressValidation';
...
const NESTED_COLLECTION = 'alternateGeographicAddress';
const NESTED_REF = 'geographicLocation';

/** What each collection's rows report as their id on the way out. */
const ROW_ID: Record<string, string> = {
  alternateGeographicAddress: 'alternateGeographicAddress-ref-0',
};

/** Owned collections, whose rows fall back to their own key with no refId. */
const REF_ID_FALLBACK = ['alternateGeographicAddress'] as const;
 ...
 */
const SORT_KEY = 'provideAlternative';
const SELECTED = 'provideAlternative';
/** One the client did not ask for, which selection must then drop. */
const NOT_SELECTED = 'validationDate';
...
const COLLECTION_OWNER: Record<string, string> = {
  alternateGeographicAddress: 'owner',
};

/** The class a ref nested inside a collection row normalises into. */
const NESTED_REF_ENTITY = GeographicLocationRefOrValue;
...
describe('GeographicAddressValidationService', () => {
  let repo: AnyRec;
  let refSave: jest.Mock;
  let manager: AnyRec;
  let dataSource: AnyRec;
  let eventEmitter: AnyRec;
  let service: GeographicAddressValidationService;
  ...
  beforeEach(() => {
    ...
    service = new GeographicAddressValidationService(
      repo as never,
      makeRepo() as never,
      makeRepo() as never,
      makeRepo() as never,
  ...
  describe('create', () => {
    ...
    it('keeps a client-supplied id and defaults @type to GeographicAddressValidation', async () => {
    ...
    it('builds from the payload the beforeCreate hook returns, not the dto', async () => {
      ...
      const HOOK_PAYLOAD: AnyRec = {
        id: 'from-hook',
        provideAlternative: 'res-from-hook',
      };
      ...
      expect(persisted.provideAlternative).toBe('res-from-hook');
    });
  });
  ...
  describe('update', () => {
    ...
    it('emits a state-change event only when the payload carries state', async () => {
      const emitted = () =>
        eventEmitter.emitEvent.mock.calls.map((c: unknown[]) => c[0]);
      ...
      manager.findOne.mockResolvedValue(entity());
      const res = await service.update(ID, { state: 'changed' } as never);
