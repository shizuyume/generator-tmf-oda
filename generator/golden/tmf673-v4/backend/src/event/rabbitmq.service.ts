...
import { TMF673_EVENT_EXCHANGE } from '../common/constants/tmf.constants';
...
export class RabbitMqService implements OnApplicationShutdown {
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      await this.channel.assertExchange(TMF673_EVENT_EXCHANGE, 'topic', {
        durable: true,
      });
      this.logger.log(
        `Connected to RabbitMQ, exchange=${TMF673_EVENT_EXCHANGE}`,
      );
