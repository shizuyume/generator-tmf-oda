...
async function bootstrap() {
  ...
  app.getHttpAdapter().get('/health', (_req, res) => {
    res.json({
      status: 'ok',
      timestamp: new Date(),
      service: 'geographic-address-management',
    });
  });

  const config = new DocumentBuilder()
    .setTitle('API Place - GeographicAddress')
    .setDescription('TMF673 API Place - GeographicAddress API v4')
    .setVersion('4.0.1')
    .build();
  ...
  // 3902 is this component's identity port ('3' + TMF number, e.g. TMF736 -> 3736);
  // PORT in .env overrides it per-environment.
  const port = Number(process.env.PORT) || 3902;
  await app.listen(port, '0.0.0.0');

  logger.log(`API Place - GeographicAddress listening on port ${port}`);
