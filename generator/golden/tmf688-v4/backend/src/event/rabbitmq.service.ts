...
import { TMF688_EVENT_EXCHANGE } from '../common/constants/tmf.constants';
...
export class RabbitMqService implements OnApplicationShutdown {
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      await this.channel.assertExchange(TMF688_EVENT_EXCHANGE, 'topic', {
        durable: true,
      });
      this.logger.log(
        `Connected to RabbitMQ, exchange=${TMF688_EVENT_EXCHANGE}`,
      );
