...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Event Streaming API')
    .setDescription('TMF688 Event Streaming API API v4')
    .setVersion('4.0.0')
    .build();
  ...
  // 3905 is this component's identity port ('3' + TMF number, e.g. TMF736 -> 3736);
  // PORT in .env overrides it per-environment.
  const port = Number(process.env.PORT) || 3905;
  await app.listen(port, '0.0.0.0');

  logger.log(`Event Streaming API listening on port ${port}`);
